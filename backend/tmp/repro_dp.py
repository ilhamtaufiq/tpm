...
from app.schemas.bengkel import TransaksiBengkelCreate, PaymentItem, DetailPartCreate, DetailServiceCreate
...
try:
    ...
    piutang = db.query(PiutangUsaha).filter(PiutangUsaha.nomor_referensi == trx.nomor_transaksi).order_by(PiutangUsaha.id.desc()).first()
    if piutang:
