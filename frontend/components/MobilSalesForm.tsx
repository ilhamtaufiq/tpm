    ...
    const renderFormContent = () => (
        <View className="px-6 pb-12 pt-4">
            ...
            <Card className="bg-background border-transparent p-4 mb-6">
                ...
                <View className="flex-row justify-between border-t border-transparent pt-3">
                    <View>
            ...
            <View className="mb-6">
                <View className="flex-row items-center justify-between mb-4">
                    ...
                    <Pressable
                        ...
                        className={`px-3 py-1.5 rounded-full ${isSplitPayment ? 'bg-amber-100 border border-amber-200' : 'bg-background border border-transparent'}`}
                    >
                ...
                {isSplitPayment ? (
                    <View className="mb-4">
                        ...
                        {payments.map((p, idx) => (
                            <View key={p.id} className="mb-3 p-4 border border-transparent rounded-2xl bg-surface/50">
                                <View className="flex-row justify-between items-center mb-3">
                                ...
                                <View className="flex-row flex-wrap gap-2 mb-3">
                                    {['TUNAI', 'TRANSFER'].map((m) => (
                                        <Pressable
                                            ...
                                            className={`flex-1 py-1.5 items-center rounded-xl border ${p.metode === m ? 'border-primary bg-primary/10' : 'border-transparent bg-surface'}`}
                                        >
                    ...
                    <>
                        ...
                        <View className="flex-row space-x-2 mb-4">
                            <Pressable
                                ...
                                className={`flex-1 py-3.5 items-center rounded-2xl border-2 ${metodeBayar === 'TUNAI' ? 'border-primary bg-primary/10 shadow-sm' : 'border-transparent'}`}
                            >
                            ...
                            <Pressable
                                ...
                                className={`flex-1 py-3.5 items-center rounded-2xl border-2 ${metodeBayar === 'TRANSFER' ? 'border-primary bg-primary/10 shadow-sm' : 'border-transparent'}`}
                            >
            ...
            <Card className="bg-primary/5 border-primary/10 p-5 mb-8">
                ...
                <View className="space-y-4">
                    <View className="flex-row justify-between">
                        ...
                        <Typography weight="bold" className="text-text">{formatCurrency(parseNumber(hargaJual) || 0)}</Typography>
                    </View>

                    <View className="flex-row justify-between">
                        <Typography variant="body2" className="text-textGray">Harga Beli Unit</Typography>
                        <Typography weight="bold" className="text-text">{formatCurrency(Number(activeUnit.harga_beli) || 0)}</Typography>
                    </View>

                    <View className="flex-row justify-between">
                        <Typography variant="body2" className="text-textGray">Biaya Pengeluaran (Pajak, BBN, dll)</Typography>
                        <Typography weight="bold" className="text-text">{formatCurrency(Number(activeUnit.total_biaya) || 0)}</Typography>
                    </View>

                    <View className="flex-row justify-between">
                        <Typography variant="body2" className="text-textGray">Biaya Sparepart dan Servis</Typography>
                        <Typography weight="bold" className="text-text">{formatCurrency(Number(activeUnit.total_part_service) || 0)}</Typography>
                    </View>

                    {totalCostsAtSale > 0 && (
                        <View className="flex-row justify-between">
                            <Typography variant="body2" className="text-textGray italic">Biaya Operasional Tambahan</Typography>
                            <Typography weight="bold" className="text-text">{formatCurrency(totalCostsAtSale)}</Typography>
                        </View>
                    )}
                    ...
                    {activeUnit.tipe_kepemilikan?.toString().toLowerCase() === 'investor' && (
                        <>
                            <View className="flex-row justify-between">
                                ...
                                <Typography weight="bold" className="text-text">{formatCurrency(Number(activeUnit.nominal_investor) || 0)}</Typography>
                            </View>
                            <View className="flex-row justify-between">
                                <Typography variant="body2" className="text-textGray">Bagian Laba Investor ({activeUnit.persentase_investor}%)</Typography>
                                <Typography weight="bold" className="text-text">{formatCurrency(labaInvestor)}</Typography>
                            </View>
