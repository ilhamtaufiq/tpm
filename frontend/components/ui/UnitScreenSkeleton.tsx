    ...
    return (
        <SafeAreaView className="flex-1 bg-surface" edges={['top']}>
            ...
            <View className="px-5 pt-4 pb-3 border-b border-border">
                ...
                <Typography className="text-xs text-textGray mt-0.5">Menyiapkan layar…</Typography>
            </View>
            <View className="px-5 mt-4 flex-row flex-wrap gap-3">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                    <View key={i} className="w-[30%] h-20 rounded-2xl bg-background" />
                ))}
            </View>
            <View className="mx-5 mt-6 h-28 rounded-3xl bg-background" />
