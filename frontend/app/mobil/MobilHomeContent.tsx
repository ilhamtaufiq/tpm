...
export default function MobilInventoryScreen() {
    ...
    const secondaryReady = useDeferredReady(80);
    ...
    /** Mount heavy form trees only when a sheet/modal needs them */
    const [mountNewForm, setMountNewForm] = useState(false);
    const [mountSalesForm, setMountSalesForm] = useState(false);
    const [mountCostForm, setMountCostForm] = useState(false);
    const [mountDetail, setMountDetail] = useState(false);
    const [mountEditForm, setMountEditForm] = useState(false);
    ...
    const handlePresentSalesModal = useCallback((unit: any) => {
        ...
        setMountSalesForm(true);
        if (Platform.OS === 'web') {
            setWebModal('sales');
        } else {
            requestAnimationFrame(() => salesBottomSheetModalRef.current?.present());
        }
    }, []);

    const handlePresentCostModal = useCallback((unit: any) => {
        setSelectedUnit(unit);
        setMountCostForm(true);
        if (Platform.OS === 'web') {
            setWebModal('cost');
        } else {
            requestAnimationFrame(() => costBottomSheetModalRef.current?.present());
        }
    }, []);
    ...
    const handlePresentDetailModal = useCallback((unit: any) => {
        ...
        if (Platform.OS === 'web') {
            ...
            requestAnimationFrame(() => detailBottomSheetModalRef.current?.present());
        }
    }, []);

    const handlePresentEditModal = useCallback((unit: any) => {
        setEditingUnit(unit);
        setMountEditForm(true);
        if (Platform.OS === 'web') {
            setWebModal('edit');
        } else {
            requestAnimationFrame(() => editBottomSheetModalRef.current?.present());
        }
    }, []);
            ...
            onConfirm: async () => {
                try {
                    ...
                    const result = await offlineAwareWrite(queryClient, {
                        type: 'mobil.delete',
                        payload: { id: unit.id },
                        label: 'Hapus unit mobil',
                        description: `${unit.merek} ${unit.model} (${unit.nomor_plat})`,
                        onlineFn: () => deleteMutation.mutateAsync(unit.id),
                    });
                    refetch();
                    closeDialog();
                    if (result.mode === 'offline') {
                        appAlert(
                            'Offline Mode',
                            'Penghapusan unit mobil dijadwalkan di antrean offline (perangkat).'
                        );
                    }
                } catch (error) {
                            ...
                            {[
                                ...
                                },
                                {
                                    ...
                                    onPress: () => {
                                        ...
                                        setExpenseNote('');
                                        setDebiturName('');
                                        setExpensePaymentMethod('KAS_UNIT_MOBIL');
                                    }
                                },
                                {
                                    key: 'HUTANG',
                        ...
                        <Button
                            ...
                            onPress={async () => {
                                ...
                                try {
                                    ...
                                    const msg = getErrorMessage(error, 'Gagal mencatat transaksi');
                                    handleCloseWallet();
            ...
            <View className="flex-1 bg-surface">
                ...
                {Platform.OS === 'web' ? (
                    ...
                    <>
                        ...
                        >
                            <View className="flex-1">
                                {mountSalesForm && selectedUnitData ? (
                                    <MobilSalesForm unit={selectedUnitData} onSuccess={() => { salesBottomSheetModalRef.current?.dismiss(); refetch(); }} />
                                ) : null}
                            </View>
                        </BottomSheetModal>
                        <BottomSheetModal
                        ...
                        >
                            <View className="flex-1">
                                {mountCostForm && selectedUnitData ? (
                                    <MobilCostForm unit={selectedUnitData} onSuccess={() => { costBottomSheetModalRef.current?.dismiss(); refetch(); }} />
                                ) : null}
                            </View>
                        </BottomSheetModal>
                        <BottomSheetModal
                        ...
                        >
                            <View style={{ flex: 1 }}>
                                {mountDetail && selectedDetailUnit ? (
                                    <MobilDetail
                                        unit={selectedDetailUnit}
                                        onClose={() => detailBottomSheetModalRef.current?.dismiss()}
                                        onSell={(u) => {
                                            detailBottomSheetModalRef.current?.dismiss();
                                            handlePresentSalesModal(u);
                                        }}
                                        onEdit={() => {
                                            detailBottomSheetModalRef.current?.dismiss();
                                            handlePresentEditModal(selectedDetailUnit);
                                        }}
                                    />
                                ) : null}
                            </View>
                        ...
                        >
                            <View className="flex-1">
                                {mountEditForm && editingUnit ? (
                                    <MobilForm initialData={editingUnit} onSuccess={() => { editBottomSheetModalRef.current?.dismiss(); refetch(); }} />
                                ) : null}
                            </View>
                        </BottomSheetModal>
