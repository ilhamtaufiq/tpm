            ...
            <SafeAreaView className="flex-1" edges={['top']}>
                ...
                <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
                    ...
                    <View className="flex-row flex-wrap -mx-2">
                        ...
                        <View className="w-1/2 px-2 mb-4">
                            <View className="bg-white/5 p-4 rounded-3xl border border-white/10">
                                ...
                                <Typography className="text-white/40 text-[9px] font-bold uppercase mt-2">Errors</Typography>
                                <Typography weight="bold" className="text-rose-500 text-xl">{errorCount}</Typography>
                            </View>
                        </View>
                        <View className="w-1/2 px-2 mb-4">
                            <View className="bg-white/5 p-4 rounded-3xl border border-white/10">
                                <Database size={16} color="#A855F7" />
                                <Typography className="text-white/40 text-[9px] font-bold uppercase mt-2">Traffic</Typography>
                                <Typography weight="bold" className="text-white text-xl">{(totalPayloadSize / 1024).toFixed(1)}<Typography className="text-xs text-white/50">KB</Typography></Typography>
                            </View>
                        </View>
                    </View>
                    ...
                    <View className="bg-white/5 p-6 rounded-[32px] border border-white/10 mb-6">
                        ...
                        <Typography variant="caption" className="text-white/30 text-[10px] mt-3">
                            Estimated impact of polling behavior on server resources and user battery.
                        </Typography>
                    </View>

                    {/* Real-time Logs List */}
                    <View className="mb-2 flex-row justify-between items-center">
                        <Typography weight="bold" className="text-white/80">Transaction Logs</Typography>
                        <Typography variant="caption" className="text-white/30">Last 100 events</Typography>
                    </View>
                    ...
                    <View className="bg-white/5 rounded-[32px] border border-white/10 overflow-hidden mb-12">
                        {logs.length === 0 ? (
                            ...
                            logs.map((log, idx) => (
                                <View 
                                    key={log.id} 
                                    className={`p-4 flex-row items-center border-b border-white/5 ${idx === 0 ? 'bg-white/5' : ''}`}
                                >
                                    <View className="mr-3">
                                    ...
                                    <View className="flex-1">
                                        <View className="flex-row items-center justify-between">
                                            <Typography variant="caption" weight="bold" className="text-white/90 font-mono text-[10px]">
                                        ...
                                        <View className="flex-row items-center mt-1">
                                            <Typography className="text-[8px] text-white/40 font-bold mr-3 italic">
                                                Status: {log.status || 'PENDING'}
                                            </Typography>
                                            {log.duration && (
                                                <Typography className="text-[8px] text-blue-400 font-bold">
